...
| 功能         | Method & Path        | 说明                   |
|--------------|----------------------|------------------------|
| 查询菜单列表 | `GET /api/drinks`    | 返回全部上架咖啡与甜品摘要信息 |

**管理接口**

| 功能         | Method & Path                 | 说明                         |
|--------------|-------------------------------|------------------------------|
| 查询菜单列表 | `GET /api/admin/drinks`       | 返回完整咖啡与甜品信息用于后台管理 |
| 新增菜单项   | `POST /api/admin/drinks`      | 创建菜单项（需要名称/价格）     |
| 更新菜单项   | `PUT /api/admin/drinks/{id}`  | 修改菜单信息                 |
| 删除菜单项   | `DELETE /api/admin/drinks/{id}` | 移除菜单项                    |
| 用户注册     | `POST /api/auth/register`     | 创建账号（支持三角色注册）   |
| 用户登录     | `POST /api/auth/login`        | 校验用户名/密码并校验角色    |
| 管理驾驶舱   | `GET /api/admin/dashboard`    | 返回菜单/门店/订单等总览指标 |
...
1. **首页**：展示今日灵感咖啡甜品卡片，并在管理员或商家登录后显示实时运营指标。
2. **点单/工作台**：根据角色切换不同功能——管理员可维护菜单，商家可处理门店订单，顾客可在线预约咖啡与甜品。
